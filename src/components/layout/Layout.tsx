  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <nav style={{ position: 'fixed', top: 32, right: 32, zIndex: 50, display: 'flex', gap: 24, padding: '12px 24px', background: 'rgba(255,248,245,0.8)', backdropFilter: 'blur(12px)', borderRadius: 9999, border: '1px solid var(--color-outline-variant)' }}>
        <NavLink to="/" style={({ isActive }) => ({ fontSize: 14, fontWeight: 500, color: isActive ? 'var(--color-secondary)' : 'var(--color-on-surface-variant)', textDecoration: 'none', borderBottom: isActive ? '2px solid var(--color-secondary)' : 'none', paddingBottom: isActive ? 2 : 0 })}>
          Home
        </NavLink>
